...
filtrar_registros <- reactive({
  ...
  registros_filtrados <-
    registros_filtrados |>
    filter(año >= input$año[1])
  
    # Filtro por año maximo
  registros_filtrados <-
    registros_filtrados |>
    filter(año <= input$año[2])
...
output$mapa <- renderLeaflet({
	# Filtrado del conjunto de datos
  registros_iguanidae <- filtrar_registros()
  
  # Mapa interactivo
  colores_regiones_especies <-
  colorNumeric(
    palette = "Reds", 
    domain = registros_union_especies_regiones$regiones_socioeconomicas, 
    na.color = "transparent"
  ) 

colores_especies <- colorFactor(
  palette = viridis(length(unique(regiones_socioeconomicas$region))),
  domain = regiones_socioeconomicas$region
)
  ...
  leaflet() |>
    ...
    fillColor = ~ colores_regiones_especies(registros_union_especies_regiones$regiones_socioeconomicas),
    fillOpacity = "black",
    stroke = TRUE,
    weight = 1.0,
    popup = paste(
...
output$grafico <- renderPlotly({
  ...
  grafico_ggplot2 <-
    ...
    st_drop_geometry() |>
    group_by(region) |>
    summarize(n = regiones_socioeconomicas) |>
